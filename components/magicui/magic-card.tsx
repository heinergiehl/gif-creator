...
const MagicContainer = ({ children, className, id }: MagicContainerProps) => {
  ...
  }, [containerRef.current, containerRef.current?.children.length]);
  useEffect(() => {
    init();
...
const MagicCard: React.FC<MagicCardProps> = ({
  ...
  return (
    <div
      ...
      className={cn(
        'relative z-0 h-full w-full rounded-sm ',
    ...
    >
      {children}
      {/* Background */}
      <div className={'absolute inset-[1px] -z-20 rounded-sm bg-white dark:bg-black/95'} />
    </div>
  );
};
