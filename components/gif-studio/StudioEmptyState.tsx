...
export function StudioEmptyState({
  ...
  return (
    <Root
      aria-label={embedded ? 'GIF editor upload' : undefined}
      className={cn(
        'flex min-h-0 flex-1 items-center justify-center overflow-auto bg-[#090d14] text-slate-100',
        embedded ? 'px-4 py-5 sm:px-6' : 'px-5 py-10',
      )}
    >
      <div className={cn('w-full', embedded ? 'max-w-5xl' : 'max-w-2xl')}>
        <div
          className={cn(
            embedded ? 'grid gap-5 md:grid-cols-[0.72fr_1.28fr] md:items-center md:gap-7' : '',
          )}
        >
          <div className={cn('mx-auto max-w-xl text-center', embedded && 'md:text-left')}>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-sky-300">
              GIF Studio
            </p>
            <Heading className="mt-3 text-balance text-2xl font-semibold tracking-[-0.035em] text-white sm:text-3xl">
              One timeline for the whole animation
            </Heading>
            <p
              className={cn(
                'mt-3 max-w-lg text-pretty text-sm leading-6 text-slate-400',
                !embedded && 'mx-auto sm:text-base',
              )}
            >
              Open a GIF, short video, screen recording, or image sequence. Adjust real frame
              timing, annotate the motion, and export without a watermark.
            </p>
          </div>

          <div
            className={cn(
              'rounded-xl border border-dashed px-5 text-center transition-colors sm:px-6',
              embedded ? 'py-6' : 'mt-9 py-9',
        ...
        <div
          className={cn(
            'grid gap-3 text-xs text-slate-500 sm:grid-cols-3',
            embedded ? 'mt-5' : 'mt-6',
          )}
        >
